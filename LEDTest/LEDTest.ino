...
LPD8806 strip = LPD8806(40, dataPin, clockPin);
LPD8806 strip2 = LPD8806(40,dataPin2, clockPin);
  ...
  for (j=0; j < 384; j++) {     // 3 cycles of all 384 colors in the wheel
    for (i=0; i < strip.numPixels(); i++) {
      strip.setPixelColor(i, Wheel( (i + j) % 384));
            strip2.setPixelColor(i, Wheel( (i + j) % 384));
    }  
...
void rainbowCycle(uint8_t wait) {
  ...
  for (j=0; j < 384 * 5; j++) {     // 5 cycles of all 384 colors in the wheel
    for (i=0; i < strip.numPixels(); i++) {
      ...
      strip2.setPixelColor(i, Wheel( ((i * 384 / strip.numPixels()) + j) % 384) );
    }  
    strip.show();   // write all the pixels out
    strip2.show();   // write all the pixels out
    delay(wait);
...
void colorChase(uint32_t c, uint8_t wait) {
  ...
  for (i=0; i < strip.numPixels(); i++) {
   ...
   for (i=0; i < strip.numPixels(); i++) {
    strip2.setPixelColor(i, 0);  // turn all pixels off
  } 
  
  for (i=0; i < strip.numPixels(); i++) {
      strip2.setPixelColor(i, c);
      if (i == 0) { 
        strip2.setPixelColor(strip2.numPixels()-1, 0);
      } else {
